...
class Qombit:
    ...
    def getMask(self) -> QomboImage:
        if self.oMask is None:
            match self.oKind:
                ...
                case OrKind.Mandel:    self.oMask = MandelbrotQomboImage()
        return self.oMask
    ...
    def canSell(self):
        """Check if this Qombit can be sold."""
        return not self.canGenerate()
    
    def getGeneratedKind(self):
        """Get the kind of Qombit generated by this one."""
        match self.oKind:
            case OrKind.Generator: return OrKind.Star
            case OrKind.Star:      return OrKind.Mandel
            case OrKind.Mandel:    return OrKind.Spiral
            case OrKind.Spiral:    return OrKind.Ring
            case OrKind.Ring:      return OrKind.Julia
            case OrKind.Julia:     return OrKind.Dice
            case OrKind.Dice:      return OrKind.Star
        return None
    ...
    def getDescription(self) -> str:
        ...
        if self.canGenerate():
            sDesc += '\nClick to create'
        if self.canSell():
            sDesc += '\nSell for ' + str(self.getPoints()) + ' points'
        return sDesc
